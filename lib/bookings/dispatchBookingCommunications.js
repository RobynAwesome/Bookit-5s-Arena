...
async function runDelivery({ bookingId, recipientType, channel, purpose, recipientAddress, send }) {
  if (!recipientAddress) {
    return { status: 'skipped', reason: `Missing ${channel} destination` };
  }
  ...
  if (!delivery) {
    return { status: 'failed', reason: 'Unable to create delivery receipt' };
  }

  if (delivery.status === 'sent') {
    return { status: 'sent', duplicateSuppressed: true, deliveryId: delivery._id.toString() };
  }
  ...
  try {
    ...
    return { status: 'failed', deliveryId: delivery._id.toString(), error: errorMessage };
  }
}
