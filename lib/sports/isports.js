...
export function getActivePremierLeagueSeasonYear(referenceDate = new Date()) {
...
export function getPremierLeagueSeasonOptions(referenceDate = new Date()) {
  const activeStartYear = getActivePremierLeagueSeasonYear(referenceDate);

  return [
    {
      year: activeStartYear,
      label: `${activeStartYear}-${String((activeStartYear + 1) % 100).padStart(2, "0")}`,
    },
  ];
}
